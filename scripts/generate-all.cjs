...
/**
 * Code Generator CLI (CommonJS version)
 ...
 */
function generateRepository(moduleName, options) {
...
`;
  ...
  const filePath = path.join(SRC_DIR, 'repositories', `${pascalName}Repository.js`);
 ...
 */
function generateService(moduleName, options) {
  const { pascalName, camelName, snakeName } = moduleName;
...
`;
  ...
  const filePath = path.join(SRC_DIR, 'services', `${pascalName}Service.js`);
 ...
 */
function generateController(moduleName, options) {
  const { pascalName, camelName, snakeName } = moduleName;
...
`;
  ...
  const filePath = path.join(SRC_DIR, 'controllers', `${pascalName}Controller.js`);
 ...
 */
function generateRoutes(moduleName, options) {
...
`;
  ...
  const filePath = path.join(SRC_DIR, 'routes', `${snakeName}.routes.js`);
 ...
 * Update routes index
 */
function updateRoutesIndex(snakeName) {
  const routesIndexPath = path.join(SRC_DIR, 'routes', 'index.js');
 ...
 */
async function generateAll(moduleName, options = {}) {
...
function main(args) {
  ...
  if (!moduleName) {
    ...
    console.log(`  npm run generate:all -- <moduleName> [options]\n`);
    ...
    console.log(`  npm run generate:all -- product`);
    console.log(`  npm run generate:all -- blog-post --force\n`);
